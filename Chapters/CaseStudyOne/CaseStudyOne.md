...
Rigth now we do not have any film in the singleton so the list of films is empty.
...
The following expression creates an instance of the film list presenter and open it. 
...
We improve the look of the film presenter by specifying columns behaviour and setting window properties.
...
- Second we configure it as readonly sending the `editable: false` message. 
...
Definining interactions between presenters is done in the `connectPresenters` method. We implement it to define that, when an element of the list is selected, we should display the information in the detail presenter.
It is worth to take some time to look at `whenSelectionChangedDo:` message.

The `whenSelectionChangedDo:` method expects a block with zero or one argument. Such argument is not the selected item directly but a more complex object that represents the selection. Indeed a selection is different in single item selection list and a multiple selection list. Therefore Spec2.0 defines the notion of selection mode under the form of subclasses of `SpAbstractSelectionMode`.
...
We update the test to keep the opened presenter in an instance variable. This allows us to define a `tearDown` method that  always closes the presenter, no matter if the test succeeds or fails.
...
Tests are addictive -- because we can change programs and check that they still works and limit our stress.
...
In Spec every presenter understands the message `addStyle:` that add a tag (a CSS class) to the receiver. 
